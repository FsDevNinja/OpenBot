...
- **Let coworkers coordinate**: built-in, provider-backed, and custom AG-UI coworkers can receive directional **Bots it may ask** grants. Remote loops call `message_bot` through the signed deployment gateway, so visibility, identity, depth, fan-out, attribution, and auditing remain server decisions.
...
- **Give people and Bots their own faces**: choose a transparent animated shape-and-colour preset that blinks and looks around, reset to the generated default, or upload, replace, and remove PNG, JPEG, WebP, or animated GIF avatars from Settings and the coworker dialog. The same face follows that identity through the sidebar, channels, recipients, handoffs, and profile surfaces; working remains a separate status badge.
...
- **Governed integrations**: `/admin/plugins` reads the live toolkit catalogue from this deployment's Composio project and offers only integrations whose OAuth application Composio manages. An administrator enables one for the workspace; each person then connects their own account, and each Bot still needs explicit tool grants. The server revalidates every selection against the live catalogue before storing it. Composio owns provider tokens; OpenBot owns grants, policy, and audit. Managed, unknown, and custom-server tools are treated as writes unless reviewed otherwise.
...
| `DEPLOYMENT_ID`                      | Names this customer deployment in thread and managed-connector identities. |
| `COMPOSIO_API_KEY`                   | Project key for this deployment's managed connectors; never share it across customers. |
