...
/**
 * Answers whether the native store has a given thread for a given person.
 *
 * Two outcomes only, and deliberately not a third: `"known"` means the thread is there, `"unknown"`
 * means PostgreSQL has clearly said it is not. A check that failed to get either answer is not a
 * value this type can hold; the
 ...
 * gone. The shape check keeps a string that could not possibly be a thread id from reaching storage.
 */
...
export function createThreadRoutes(
  ...
  /**
   * Absent when this deployment has no way to ask its thread store. `GET /:threadId` is then not registered at all, rather than
  ...
  if (readThread) {
     ...
     * remembers from last time is one PostgreSQL still has. If the answer is no, it can start a
     * fresh thread knowing there is provably nothing left in the old one to lose. Without this route
     * that is not what happens — the browser sends anyway, the runtime starts a new, empty
      ...
      try {
        ...
        console.error(
          JSON.stringify({
            ...
            note: "Could not determine whether the native runtime still has this thread.",
          }),
        );
